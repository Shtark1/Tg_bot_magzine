    ...
    if callback.data == "CHECK_PAY":
        ...
        if not history.operations:
            ...
            if callback.data in ["strat_1", "strat_2", "strat_3", "strat_4"]:
                await bot.send_photo(callback.from_user.id,photo=f"img/{callback.data}.jpg", caption=MESSAGES[f"{all_data['tovar']}_tovar"], reply_markup=BUTTON_TYPES["BTN_HOME"])
            elif callback.data == "strat_5":
                await bot.send_photo(callback.from_user.id, photo=f"img/strat_1.jpg", caption=MESSAGES["strat_1_tovar"], reply_markup=BUTTON_TYPES["BTN_VIP_1"])
                await bot.send_photo(callback.from_user.id, photo=f"img/strat_3.jpg", caption=MESSAGES["strat_3_tovar"], reply_markup=BUTTON_TYPES["BTN_VIP_1"])
            elif callback.data == "strat_6":
                await bot.send_photo(callback.from_user.id, photo=f"img/strat_2.jpg", caption=MESSAGES["strat_2_tovar"], reply_markup=BUTTON_TYPES["BTN_VIP_1"])
                await bot.send_photo(callback.from_user.id, photo=f"img/strat_4.jpg", caption=MESSAGES["strat_4_tovar"], reply_markup=BUTTON_TYPES["BTN_VIP_1"])
